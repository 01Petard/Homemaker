...
import static com.wjx.homemaker.R.id.ll_itemview;


public class ExpressMapActivity extends AppCompatActivity implements View.OnClickListener, LocationSource, AMapLocationListener, AMapNaviListener {

    private static double l,d;
    private TextView tvEnd;
    private ImageView imageView;
    private int navigationType = 0;
    private String oid_post;
    private AMap amap;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent!=null){
            oid_post=intent.getStringExtra("oid");
        }
    ...
    private void initView() {
        ...
        mAdapter = getAdapter();
        imageView= (ImageView) findViewById(R.id.rl_iv_back);
        tvEnd = (TextView) findViewById(R.id.rl_tv_end);
        tvEnd.setOnClickListener(this);
        imageView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ExpressMapActivity.this.finish();
            }
        });
    ...
    private void initMap() {
        ...
        myLocationStyle.radiusFillColor(255);//设置定位蓝点精度圆圈的填充颜色的方法。
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                while (true){
                httpUtils.send(HttpRequest.HttpMethod.POST, Config.BASEHOST+"/mobile/LocationLogServlet?method=getLocation&oid="+oid_post, new RequestCallBack<Object>() {
                    @Override
                    public void onSuccess(ResponseInfo<Object> responseInfo) {
                        try {
                            JSONObject ret=new JSONObject(responseInfo.result.toString());
                            JSONObject jsonObject=ret.getJSONObject("orderEntity");
                            System.out.println("获取快递状态:"+responseInfo.result);
                            l=jsonObject.getDouble("l");
                            d=jsonObject.getDouble("d");
                            System.out.println("l,d:"+l+","+d);
                            Tip tip=new Tip();
                            tip.setDistrict(jsonObject.getString("address"));
                            LatLonPoint endLp = new LatLonPoint(d,l);
                            tip.setPostion(endLp);
                            clearRoute();
                            tvEnd.setText("我的收货地址：     "+tip.getDistrict());
                            endList.clear();
    ...
    private void clickNavigation() {
        if(startList.size()==0){
            ...
            if (!calculateSuccess) {
                ...
                Intent gpsintent = new Intent(this, MapNaviViewActivity.class);
